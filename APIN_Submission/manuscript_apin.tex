...
\abstract{Deep Convolutional Neural Networks (CNNs) have achieved remarkable success, yet their computational intensity remains a bottleneck for deployment in edge systems. Structured pruning addresses this by removing redundant filters, but traditional metrics typically rely on local structural heuristics—such as weight magnitude or feature map rank—that overlook the \textbf{global semantic logic} within the network's reasoning process. In this paper, we introduce \textbf{GRA-CNN}, a novel structured pruning framework guided by \textbf{Gray Relational Analysis (GRA)} to measure the semantic alignment between internal activations and decision-level outputs. Drawing inspiration from the \textbf{Information Bottleneck (IB)} principle, we treat pruning as a process of preserving mutual information between internal representations and final labels. Unlike previous methods, GRA-CNN quantifies the functional coupling between feature-map activation sequences and classification logits, identifying \textbf{semantic filters} that are indispensable for discrimination. Extensive validation on CIFAR-10, CIFAR-100, and Tiny-ImageNet-200 benchmarks demonstrates that GRA-CNN consistently surpasses state-of-the-art baselines. Notably, on the challenging 200-class Tiny-ImageNet dataset, our method achieves a remarkable \textbf{+2.42\% accuracy gain} over L1-Norm at 50\% pruning, revealing that semantic alignment is a superior signal for the principled and robust compression of deep models.}
...
\vspace{0.5em}
Surprisingly, despite extensive pruning research, this \emph{sequence-level semantic alignment} has not been exploited. We define this discrepancy as the \textbf{Semantic Paradox} in model compression: a channel with high weight energy may encode redundant structural patterns (e.g., background textures), while a seemingly "weak" channel with modest activations may preserve critical, fine-grained semantic signals necessary for decision-making. 

\vspace{0.5em}
\noindent\textbf{Our idea: semantic relational pruning through Gray Relational Analysis.}  
We propose \textbf{GRA-CNN}, a comprehensive pruning framework that identifies \textbf{semantic filters} by evaluating the functional coupling between internal representations and decision-level output logits. Unlike previous methods, our framework treats each filter as an information-carrying sequence and utilizes \textbf{Gray Relational Analysis (GRA)}—a tool from grey system theory designed for trend-based similarity—to measure how well a channel's behavior aligns with the network's reasoning trajectory.
...
To capture inter-channel dependencies, recent works have explored global modeling. ACP~\cite{chang2022acp} uses clustering and swarm intelligence to search for optimal pruning configurations. More recently, Dong et al.~\cite{dong2024closeness} introduced closeness centrality from graph theory to identify redundant channels, while Xue et al.~\cite{xue2024rsga} proposed reverse search genetic algorithms for automatic layer-wise pruning. Dynamic pruning methods~\cite{hu2023dynamic_pruning,chen2021dynamical} and GCN-based approaches~\cite{jiang2022channel} model channel relationships using graphs or attention mechanisms~\cite{cheng2023differentiable}. Automated methods like ACPLI~\cite{treszczotko2021acpli} learn importance scores end-to-end. \textbf{DepGraph}~\cite{fang2023depgraph}, a landmark work in CVPR 2023, resolves structural dependency issues in any network through dependency graph analysis, enabling truly general-purpose structured pruning. While these methods capture higher-order interactions, they often introduce significant computational overhead and complex hyperparameter tuning. Our GRA-CNN offers a simpler, lightweight alternative by directly measuring semantic alignment without complex graph construction.

\subsection{Semantic and Information-Theoretic Metrics}
The frontier of pruning research focuses on aligning channel selection with semantic information. \textbf{X-Pruner}~\cite{yu2023xpruner} (ICCV 2023) utilizes explainable units to measure the contribution of channels to specific classes. Information-theoretic approaches use Mutual Information (MI)~\cite{lu2024enhancing}, Conditional MI~\cite{vuvan2024pruning}, or Information Bottleneck principles~\cite{mip2024mutual,dutta2024structured} to measure the information flow from input to output. ITPCC~\cite{rostami2025information} considers the coupled information of channel groups. While theoretically rigorous, estimating MI in high-dimensional spaces is computationally expensive and sensitive to estimation bias. Our relational approach via GRA offers a efficient surrogate that captures trend-based semantic alignment without requiring explicit probability density estimation.
...
Gray Relational Analysis (GRA) naturally satisfies these properties, providing a principled mechanism for comparing relational similarity between sequences. From an \textbf{Information Bottleneck} perspective, GRA acts as a filter that maximizes the compression of structural noise while preserving the mutual information between internal feature sequences and the target decision logic.
...
\subsection{A Holistic Relational Metric: GRA-Fisher Fusion}
While GRA captures decision-level alignment, a truly robust pruning criterion should also consider gradient sensitivity and feature diversity. We extend our framework into a \textbf{Multi-objective Relational Fusion (MORF)} metric that combines four orthogonal importance signals:

\begin{enumerate}
    \item \textbf{Class-Balanced Fisher Information ($\mathcal{I}_F$):} Measures the sensitivity of the loss landscape to the channel's presence, weighted by class distribution to ensure fairness across minority categories.
    \item \textbf{Gray Relational Grade ($\gamma_c$):} Captures the semantic alignment between activations and class-specific logits.
    \item \textbf{Channel Orthogonality ($\mathcal{O}_c$):} Penalizes redundant channels that are highly correlated with others in the same layer, encouraging feature diversity.
    \item \textbf{Structural Magnitude ($\mathcal{S}_{L1}$):} Incorporates the classical L1-Norm as a base stability term for filtering extremely small weights.
\end{enumerate}

The final importance score is computed as a weighted fusion:
\begin{equation}
\omega_c = \alpha \hat{\mathcal{I}}_F + \beta \hat{\mathcal{O}}_c + \delta \hat{\gamma}_c + \eta \hat{\mathcal{S}}_{L1},
\label{eq:fusion}
\end{equation}
where hats denote min-max normalization within each layer, and coefficients satisfy $\alpha + \beta + \delta + \eta = 1$. In our default configuration, we prioritize semantic signals ($\alpha=0.4, \delta=0.25$) while maintaining structural stability and variety.

\begin{algorithm}[H]
\caption{GRA-CNN Structured Channel Pruning (Full Framework)}
\label{alg:gra_pruning}
\begin{algorithmic}[1]
\Require Pre-trained model $\mathcal{M}$, mini-batch $\mathcal{D}_B$, pruning ratio $P$, fusion coefficients $\{\alpha, \beta, \delta, \eta\}$.
\Ensure Pruned and fine-tuned model $\mathcal{M}'$.
\State Forward pass batch $\mathcal{D}_B$ through $\mathcal{M}$;
\State Compute Fisher Information $\mathcal{I}_F$ via backpropagation;
\State Compute Gray Relational Grades $\gamma_c$ via Eq.~\eqref{eq:gra_score};
\State Compute Channel Orthogonality scores $\mathcal{O}_c$ via activation correlation;
\For{each convolutional layer $L_k$}
    \State Compute fused scores $\omega_c$ via Eq.~\eqref{eq:fusion};
    \State Normalize $\omega_c$ and identify indices of lowest $P\%$ channels;
    \State Prune filter weights and resolve structural dependencies (e.g., via DepGraph~\cite{fang2023depgraph});
\EndFor
\State Fine-tune $\mathcal{M}'$ to recover performance;
\end{algorithmic}
\end{algorithm}
...
\paragraph{Depth Scalability (The "Deep Advantage").}
Our specific "Depth Trend" analysis (Figure \ref{fig:main_results}(a)) reveals a linear correlation between network depth and GRA's advantage margin. At an extreme pruning ratio of 0.8, GRA-CNN outperforms L1-Norm by an increasing margin: \textbf{+0.5\%} on ResNet-20, rising to \textbf{+3.8\%} on ResNet-110. This confirms our hypothesis that gradient-aligned semantic stability is pivotal for deep feature propagation.

\paragraph{Extreme Sparsity (The "Breaking Point").}
Under the most rigorous test condition (ResNet-110 at 90\% sparsity), traditional L1-Norm collapses to near-random performance ($<20\%$). In contrast, GRA-CNN retains a remarkable \textbf{76.4\%} top-1 accuracy (Figure \ref{fig:main_results}(b)), demonstrating that the semantic backbone of the network remains functional even when 9 out of 10 channels are removed.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig4_advantage.png}
\caption{Advantage-Focused Performance Analysis. (a) Depth Scalability: GRA's advantage over L1 grows linearly with network depth at 80\% sparsity. (b) Extreme Sparsity: On ResNet-110, GRA maintains functional accuracy even at 90\% pruning ratio where L1 collapses. (c) Generalization: Superior performance on Tiny-ImageNet (MobileNetV2).}
...
Our experiments show that GRA is particularly effective under high pruning ratios and on datasets with complex semantic structure (e.g., CIFAR-100, Tiny-ImageNet-200). In these settings, structural metrics misinterpret representational redundancy, whereas semantic alignment directly reflects usefulness for discrimination. This observation aligns with the \textbf{Information Bottleneck} theory, suggesting that GRA-CNN successfully identifies and discards features that do not contribute to the mutual information between the network's internal state and the labels.

\paragraph{Stability of Logits as Semantic Guidance.}
A potential concern in semantic-driven pruning is the stability of the output logits used as teachers. If the logits are noisy or fluctuate significantly during early training, the resulting importance scores might be unreliable. However, our stability analysis (Section~\ref{sec:experiments}) shows that GRA relational grades remain remarkably consistent across different mini-batches and random seeds. This robustness stems from the fact that GRA measures \emph{trend similarity} rather than pointwise value matching, making it less sensitive to local fluctuations in logit magnitudes.

\paragraph{Resolving the Semantic Paradox.}
The near-zero correlation between GRA and L1-Norm (as discussed in Section \ref{sec:mechanism}) confirms that our method resolves the \textbf{Semantic Paradox}. By explicitly selecting for \textbf{semantic filters}, we ensure that the network's reasoning backbone remains intact even when aggressive structural sparsity is imposed. This shift from magnitude-based heuristics to functional relational analysis provides a more principled foundation for structured pruning.
...
\paragraph{Ablation of Multi-objective Fusion Coefficients.}
The effectiveness of GRA-CNN stems from its ability to fuse decision-level semantics with gradient sensitivity and feature diversity. We analyze the impact of the fusion coefficients $\alpha, \beta, \delta, \eta$ defined in Eq.~\eqref{eq:fusion}. Our empirical study (see Table~\ref{tab:fusion_ablation} for details) reveals that Fisher Information ($\alpha=0.4$) provides a strong base for gradient-guided importance, while Gray Relational Grades ($\delta=0.25$) introduce the critical semantic alignment signal. The inclusion of Channel Orthogonality ($\beta=0.2$) helps mitigate feature redundancy, particularly in overparameterized layers. Interestingly, even a small contribution from L1-Norm ($\eta=0.15$) serves as an effective "floor" for structural stability. This holistic combination (GRA-Fisher v2.0) consistently outperforms any single criterion, confirming that relational semantics and structural heuristics are complementary signals for robust pruning.
...
Future work will extend semantic relational scoring to Transformers, multi-modal architectures, and dynamic inference systems, deepening our understanding of representation semantics under model compression. Additionally, we plan to explore adaptive per-layer pruning ratios guided by GRA score distributions, and to scale our validation to large-scale datasets such as ImageNet-1K.
