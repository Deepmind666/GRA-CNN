...
\caption{Cross-metric correlation analysis between L1-Norm (structural) and GRA score (semantic) on ResNet-56. Each point represents a single convolutional channel. The near-zero Pearson correlation coefficient ($r = 0.015$) indicates that the two metrics are nearly orthogonal. The quadrant analysis reveals 28\% of channels as ``hidden semantic'' (Low L1, High GRA) that would be incorrectly pruned by magnitude-based methods, and 22\% as ``false positives'' (High L1, Low GRA) that would be incorrectly preserved.}
...
As shown in Table~\ref{tab:ablation}, GRA-CNN demonstrates competitive or superior performance across most configurations. Notably, the standard deviations ($\pm\sigma$) indicate that GRA exhibits comparable stability to other methods, validating the reproducibility of our approach. The Pareto frontier analysis (Figure~\ref{fig:pareto}) further illustrates that GRA consistently achieves favorable accuracy-efficiency trade-offs.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig_pareto.pdf}
\caption{Pareto frontier analysis on CIFAR-10 and CIFAR-100 using ResNet-56. Error bars indicate standard deviation across $N=3$ independent runs. GRA-CNN (red) achieves competitive accuracy-efficiency trade-offs compared to L1-Norm, FPGM, and HRank baselines.}
\label{fig:pareto}
\end{figure}
