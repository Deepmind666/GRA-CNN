...
def train(train_loader, model, criterion, optimizer, epoch, device):
    ...
    print(f'Epoch [{epoch+1}] Train Loss: {running_loss/len(train_loader):.4f} Acc: {acc:.2f}%')
...
def validate(val_loader, model, criterion, device):
    ...
    acc = 100. * correct / total
    print(f'Val Loss: {running_loss/len(val_loader):.4f} Acc: {acc:.2f}%')
    return acc

def prune_model(model, ratio, score_type, train_loader, device):
    print(f"Pruning model with {score_type} score, ratio={ratio}...")
    ...
    gra_scorer = GrayRelationalChannelScorer()
...
def main():
    ...
    # 2. Pruning
    model = prune_model(model, args.prune_ratio, args.score_type, trainloader, device)
    ...
    with open('results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([args.arch, args.score_type, args.prune_ratio, acc])
